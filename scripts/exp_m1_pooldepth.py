"""M1: subir retrieval_pool_depth 50 -> 100 (mete mas candidatos al reranker BGE).
...
Uso (env limpio, ver logs/run_m1.sh):
  PYTHONPATH=. BGE_DEVICE=cuda POOL=100 venv/bin/python -m scripts.exp_m1_pooldepth
"""
...
OUTDIR = Path("data/eval/results/m1_pooldepth")
...
def main():
    ...
    print(f"=== FASE A: re-retrieval pool={POOL} sobre {len(base)}q, detecta cambios ===", flush=True)
    ...
    print("\n=== M1 (pool={}) vs BASELINE (pool=50) ===".format(POOL), flush=True)
    cats = {}
    for c in base:
