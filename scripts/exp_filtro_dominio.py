"""E1 — filtrar_fuera_dominio: sacar del pool los fragmentos de normas ajenas.
...
NAME = os.environ.get("NAME", "filtro_dominio")
...
def main():
    ...
    def arm(qtext, gs, val, q_row=None):
        ...
        # config adoptada SIEMPRE ON; lo que se togglea es el FALLBACK de regex.
        cfg.settings.glossary_lookup = True
        cfg.settings.intent_gate = True
        cfg.settings.ambiguity_disclose = True
        cfg.settings.filtrar_fuera_dominio = val
        for _ in (1, 2, 3):
            try:
                t0 = time.time()
    ...
    print(f"=== {NAME}: {len(rows)} pares  OFF(sin filtro) / ON(filtra ajenas) ===", flush=True)
    nq = 0
    ...
    rp.write_text(json.dumps({"detail": rows}, ensure_ascii=False, default=str))
    cfg.settings.filtrar_fuera_dominio = False
