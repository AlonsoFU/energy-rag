#!/usr/bin/env python3
"""
Descargar vinculaciones pendientes, solo las relevantes al sector eléctrico.
"""
...
KEYWORDS_SECTOR = [
    'electri', 'energi', 'energía', 'transmisi', 'distribuci', 'generaci',
    'potencia', 'tarifa', 'peaje', 'coordinador', 'medidor', 'suministro',
    'kwh', 'mwh', 'cne', 'sec', 'cdec', 'transferencia', 'balance',
    'sistema electr', 'sistema energ', 'voltaje', 'sscc', 'servicios complementarios'
]
...
async def obtener_info_norma(id_norma: str, page) -> dict:
    ...
    try:
        ...
        es_relevante = any(kw in texto_lower for kw in KEYWORDS_SECTOR)

        if not es_relevante:
            return {'id_norma': id_norma, 'no_relevante': True, 'titulo': titulo[:100]}
...
async def descargar_lote(ids_lote, num_lote):
    print(f"\n[Lote {num_lote}] Revisando {len(ids_lote)} normas...")
    nuevas = {}
    no_relevantes = 0
    ...
    async with async_playwright() as p:
        ...
        for i, id_norma in enumerate(ids_lote, 1):
            ...
            if norma.get('no_relevante'):
                print(f"  [{i}/{len(ids_lote)}] ○ {id_norma} [no sector eléctrico]")
                no_relevantes += 1
            elif 'error' not in norma:
                print(f"  [{i}/{len(ids_lote)}] ✓ {norma['tipo']} {norma['numero']} [ELÉCTRICO]")
                nuevas[id_norma] = norma
            else:
                print(f"  [{i}/{len(ids_lote)}] ✗ {id_norma}")

            await asyncio.sleep(0.5)

        await browser.close()

    print(f"  → Relevantes: {len(nuevas)}, No relevantes: {no_relevantes}")
    return nuevas
...
async def main():
    ...
    print(f"Pendientes a revisar: {len(pendientes)}")
    ...
    for num, lote in enumerate(lotes, 1):
        nuevas = await descargar_lote(lote, num)
        total_relevantes += len(nuevas)
    ...
    print(f"\n{'='*60}")
    print(f"TOTAL FINAL: {len(normas)} normas")
    print(f"Nuevas relevantes agregadas: {total_relevantes}")
