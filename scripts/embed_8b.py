...
Puebla fragmentos.embedding_8b (vector 2560) con el contextual_text de cada chunk.
...
def main():
    with with_connection() as c, c.cursor() as cur:
        cur.execute("SELECT id, contextual_text FROM fragmentos "
                    "WHERE contextual_text IS NOT NULL AND embedding_8b IS NULL ORDER BY id")
        rows = cur.fetchall()
    total = len(rows)
    ...
    for i in range(0, total, B):
        ...
        with with_connection() as c, c.cursor() as cur:
            for (fid, _), e in zip(batch, embs):
                cur.execute("UPDATE fragmentos SET embedding_8b = %s::vector WHERE id = %s",
                            (str(e), fid))
            c.commit()
        done += len(batch)
