#!/usr/bin/env python3
"""
Descargar vinculaciones pendientes, filtrando solo las del sector eléctrico/energía.
"""
...
async def obtener_info_norma(id_norma: str, page) -> dict:
    ...
    try:
        ...
        # Verificar si es relevante al sector eléctrico
        es_relevante = any(kw in texto_lower for kw in KEYWORDS_ELECTRICO)
        ...
        return {
            ...
            'url': url,
            'es_sector_electrico': es_relevante
        }
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, id_norma in enumerate(sorted(pendientes), 1):
            ...
            if 'error' not in norma:
                if norma.get('es_sector_electrico', False):
                    print(f"✓ {norma['tipo']} {norma['numero']} [ELÉCTRICO]")
                    nuevas[id_norma] = norma
                else:
                    print(f"○ {norma['tipo']} {norma['numero']} [no eléctrico]")
                    no_relevantes += 1
                    # Guardar igual pero marcado
                    norma['es_sector_electrico'] = False
                    nuevas[id_norma] = norma
            else:
    ...
    normas_electricas = {k: v for k, v in normas.items()
                         if v.get('es_sector_electrico', True) or
                         v.get('temas_detectados', [])}

    output = {
        'fecha': datetime.now().isoformat(),
        'total': len(normas),
        'sector_electrico': len(normas_electricas),
    ...
    print(f"Sector eléctrico: {len(normas_electricas)}")
    print(f"No relevantes: {no_relevantes}")
    print("=" * 60)
