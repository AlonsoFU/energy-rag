"""R5 — ¿aporta algo el regex de fallback, ahora que el diccionario va primero?
...
NAME = os.environ.get("NAME", "r5_fallback")
...
def main():
    ...
    def arm(qtext, gs, val, q_row=None):
        ...
        cfg.settings.regex_fallback = val
        for _ in (1, 2, 3):
            try:
                t0 = time.time()
    ...
    print(f"=== {NAME}: {len(rows)} pares  OFF(sin fallback) / ON(con fallback regex) ===", flush=True)
    nq = 0
    ...
    rp.write_text(json.dumps({"detail": rows}, ensure_ascii=False, default=str))
    cfg.settings.regex_fallback = True
